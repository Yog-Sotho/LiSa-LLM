...
#include <chrono>
#include <thread>
#include <openssl/crypto.h>  // surgical: CRYPTO_memcmp
#include <fstream>
#include <iomanip>
#include <sstream>
#include <mutex>
...
void HttpServer::start() {
    Server svr;
    svr.set_payload_max_length(1048576);  // surgical MEDIUM fix (payload limit)

    // surgical CORS (MEDIUM)
    ...
    // surgical production-grade token-bucket rate limiter (per-key)
    struct RateLimiter {
    ...
    svr.Get("/metrics", [](const Request&, Response& res) {  // surgical new feature
    ...
    svr.Post("/v1/completions", [this](const Request& req, Response& res) {
        ...
        if (!cfg_.api_key.empty()) {
            ...
            // surgical HIGH: constant-time comparison (replaces original !=)
        ...
        if (!limiter.consume()) {  // surgical rate limit
            res.status = 429;
        ...
        try {
            ...
            for (const auto& token : tokens) {
                ...
                // surgical CRITICAL: use chunked provider (replaces broken res.send)
    ...
    // surgical new feature: full OpenAI /v1/chat/completions (added at end of start())
